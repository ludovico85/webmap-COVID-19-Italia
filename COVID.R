...
chart0<-ggplot(data=data_chart0, aes(x=data, y=percentuale)) +
  ...
  labs(x = "data", y = "%")+scale_x_date(date_breaks = "30 day",
                                         date_labels = "%b %d")+
...
chart1<-ggplot(data=data_chart1, aes(x=data, y=value, color=variable)) +
  ...
  labs(x = "data", y = "numero di casi")+scale_x_date(date_breaks = "30 day",
                                                            date_labels = "%b %d",
...
data_chart5<-regioni
data_chart5<-data_chart5[, c(1, 5, 19)]
...
casi_totali<-regioni_dati[, c(5, 19)]
...
### eliminazione vecchi files
file.remove(file.path(paste(getwd(),'/webmap/dati', sep = ""), list.files(paste(getwd(),'/webmap/dati', sep = ""))))
